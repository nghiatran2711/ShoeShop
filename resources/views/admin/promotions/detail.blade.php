...
@push('js')
    ...
    // set Begin Date 1
        ...
        if (!empty($begin) && !empty($end)) {
            $beginDate1 = date('Y-m-d 00:00:00', strtotime($begin));
            // set Begin Date 2
            $beginDate2 = date('Y-m-d H:i:s', strtotime($end));
        }else {
            $beginDate1 = date('Y-m-d 00:00:00');
            // set Begin Date 2
                    ...
                    <form action="{{ route('admin.promotion.update',['id'=>$promotion->id]) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-xs-6">
                                ...
                                <input type="text" name="name" value="{{ old('name', $promotion->name) }}" class="form-control" disabled>
                            </div>
                            <div class="col-xs-6">
                                <label for="">Discount</label>
                            <input type="number" name="discount"  value="{{ old('discount', $promotion->discount) }}" class="form-control" disabled>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-xs-6">
                                <label for="">Quantity</label>
                                <input type="number" name="quantity" value="{{ old('quantity', $promotion->quantity) }}" class="form-control" disabled>
                            </div>
                            <div class="col-xs-6">
                                <label for="">Status</label>
                                <br>
                                <input type="checkbox" name="status" placeholder="Status" class="" checked value="1" disabled>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-xs-6">
                                <label for="">Begin Date</label>
                                <input type="text" name="begin_date" value="{{ old('begin_date', $promotion->begin_date) }}" class="form-control dt-begin-date" disabled>
                            </div>
                            <div class="col-xs-6">
                                <label for="">End Date</label>
                                <input type="text" name="end_date" value="{{ old('end_date', $promotion->end_date) }}" class="form-control dt-end-date" disabled>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-xs-12">
                                <label for="">List Product</label>
                                  <select class="form-control select2-list-product" name="list_product[]" multiple="multiple" disabled>
